...
const PrintButton = ({ formData, onClearForm, onNewForm, onValidate, errors, onFormSaved }) => {
  ...
  const handleSavePDF = async () => {
    ...
    try {
      ...
      // Obtener la altura real del contenido completo
      ...
      const canvas = await html2canvas(tempContainer, {
        ...
        backgroundColor: '#ffffff',
        width: 794,
        height: contentHeight, // Usar altura completa del contenido
        windowHeight: contentHeight, // Forzar captura completa
        onclone: (clonedDoc) => {
          // Asegurar que todos los estilos se apliquen correctamente
          const allElements = clonedDoc.querySelectorAll('*');
          allElements.forEach(el => {
      ...
      // Ajustar el contenido para que SIEMPRE quepa en una página
      let finalWidth = imgWidth;
      let finalHeight = imgHeight;

      // Si la imagen es más alta que la página, escalarla proporcionalmente
      ...
      // Centrar horizontalmente si se redujo el ancho
      const xOffset = (pdfWidth - finalWidth) / 2;

      // Añadir la imagen al PDF ajustada a una sola página
